...
function buildSqlQueryForCodes(codeParam) {
  ...
  if (codeParam) {
    // if 'code' URL param was supplied
...
function buildSqlQueryForNeighborhoods(idParam) {
  ...
  if (idParam) {
    // if 'id' URL param was supplied
    ids = idParam.split(",");
    sql +=
      " WHERE neighborhood_number = " + ids.join(" OR neighborhood_number = ");
  }
...
function formatResponse(responseObject, formatParam) {
  ...
  if (formatParam === "XML") {
    response = xmlConverter.js2xml(responseObject, {
      compact: false,
      spaces: 4
    });
